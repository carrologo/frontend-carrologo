...
export const ModalEditTransaction: React.FC<ModalEditTransactionProps> = ({
  ...
  const [initialValues, setInitialValues] =
    useState<UpdateTransactionPost | null>(null);
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      setInitialValues({
        ...
        id_status:
          transaction.statusInfo?.id_status || transaction.id_status || 1,
      });
  ...
  const handleSubmit = async (values: UpdateTransactionPost) => {
    ...
    try {
      ...
      if (error.response?.status === 413) {
        showErrorToast(
          "El archivo es demasiado grande. Por favor, seleccione un archivo más pequeño."
        );
  ...
  const findSelectedClient = (clientId: number | null): Client | null => {
    ...
    return (
      clients.find((client) => Number(client.id) === Number(clientId)) || null
    );
  };
  ...
  const findSelectedVehicle = (vehicleId: number | null): Vehicle | null => {
    ...
    return (
      vehicles.find((vehicle) => Number(vehicle.id) === Number(vehicleId)) ||
      null
    );
  };
                ...
                <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
                  ...
                  >
                    ...
                    <Autocomplete
                      options={vehicles}
                      getOptionLabel={(option) =>
                        `${option.brand} ${option.line} - ${option.plate}`
                      }
                        ...
                        <TextField
                          ...
                          placeholder={
                            transactionData?.vehicleInfo
                              ? `${transactionData.vehicleInfo.description} - ${
                                  transactionData.vehicleInfo.plate ||
                                  "Sin placa"
                                }`
                              : "Buscar vehículo..."
                          }
                    ...
                    <Autocomplete
                      ...
                      value={
                        Object.entries(transactionStatusMap)
                          .map(([id, name]) => ({ id, name }))
                          .find(
                            (option) => option.id === String(values.id_status)
                          ) || null
                      }
                      onChange={(_, newValue) => {
                        setFieldValue(
                          "id_status",
                          Number(newValue?.id) || values.id_status
                        );
                      }}
                      renderInput={(params) => (
                        <TextField
