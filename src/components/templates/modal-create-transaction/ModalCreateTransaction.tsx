...
import {
  cleanBase64,
  getBase64Size,
} from "../../../utils/documentCompression.utils";
    ...
    try {
      ...
      if (
        ...
        if (totalSize > maxTotalSize) {
          throw new Error(
            `El tamaño total de los documentos (${(
              ...
              1024 /
              1024
            ).toFixed(2)}MB) excede el límite de ${
        ...
        processedDocuments = data.documents.map((doc: any) => {
          ...
          if (docSize > maxFileSize) {
            throw new Error(
              `El documento "${doc.name}" (${(docSize / 1024 / 1024).toFixed(
                2
              )}MB) excede el límite de ${
      ...
      const cleanData: CreateTransactionPost = {
        id_vehicle: data.id_vehicle
          ? parseInt(data.id_vehicle.toString())
          : null,
      ...
      console.log("Datos a enviar:", {
        ...
        documents: cleanData.documents
          ? `[${cleanData.documents.length} documentos]`
          : null,
      });
      ...
      console.log("Validaciones:", {
        ...
        id_vehicle_valid:
          cleanData.id_vehicle === null || !isNaN(cleanData.id_vehicle),
        id_buyer_valid:
          cleanData.id_buyer === null || !isNaN(cleanData.id_buyer),
        id_seller_valid:
          cleanData.id_seller === null || !isNaN(cleanData.id_seller),
        amount_valid: cleanData.amount === null || !isNaN(cleanData.amount),
        documents_valid:
          cleanData.documents === null ||
          (Array.isArray(cleanData.documents) &&
            cleanData.documents.length > 0),
      ...
      if (error && typeof error === "object" && (error as any).response) {
        ...
        if (errorData?.message) {
          ...
          alert(
            "Error interno del servidor. El documento puede ser demasiado grande. Intente con archivos más pequeños."
          );
        } else if (errorResponse.status === 413) {
          alert(
            "Los documentos son demasiado grandes. Reduzca el tamaño de los archivos e intente nuevamente."
          );
        }
            ...
            <DialogContent>
              ...
              >
                ...
                <Box>
                  <Autocomplete
                    options={vehicles.map((vehicle) => ({
                      ...
                      label: `${vehicle.brand} ${vehicle.line} ${
                    ...
                    value={
                      vehicles
                        ...
                        .find((option) => option.value === values.id_vehicle) ||
                      null
                    }
                    onChange={(_, newValue) =>
                      setFieldValue("id_vehicle", newValue?.value || "")
                    }
                    renderInput={(params) => (
                      <TextField
                        {...params}
                  ...
                  <Autocomplete
                    ...
                    value={
                      clients
                        ...
                        .find((option) => option.value === values.id_buyer) ||
                      null
                    }
                    onChange={(_, newValue) =>
                      setFieldValue("id_buyer", newValue?.value || "")
                    }
                    renderInput={(params) => (
                      <TextField
                        {...params}
                  ...
                  <Autocomplete
                    ...
                    value={
                      clients
                        ...
                        .find((option) => option.value === values.id_seller) ||
                      null
                    }
                    onChange={(_, newValue) =>
                      setFieldValue("id_seller", newValue?.value || "")
                    }
                    renderInput={(params) => (
                      <TextField
                        {...params}
                ...
                <Box>
                  <FormControl
                    fullWidth
                    error={touched.id_status && Boolean(errors.id_status)}
                  >
                    <InputLabel>Estado de transacción *</InputLabel>
                    <Select
                      ...
                      label="Estado *"
                    >
                      {Object.entries(transactionStatusMap).map(
                        ([id, name]) => (
